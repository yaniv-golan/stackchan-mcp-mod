...
- **The AI is the operator, and it can be talked into things.** Text an assistant reads — a web page, a file, an
  ...
  `take_photo`, `listen`, `record_and_play`, `get_recorded_audio` and `restart_robot` under `permissions.ask`, and
  never mark them always-allow. This is the single most effective control against that path, and it is a
